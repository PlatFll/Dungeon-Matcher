...
- `CombatIdleImporter.ImportGuards` selects the restored pre-correction sources in `ArtSource/GuardIdles/` for Crossbow Guard, Barricade Guard, Spear Guard and Siege Sergeant only. It reuses the existing nine-frame/130ms Point/FullRect import and definition-selected controllers; no new runtime owner is added. Only fallback sprite/controller references change. The guards retain generic action feedback and existing gameplay timing. `CombatIdleValidation.ImportGuardsAndRun` verifies the selected bytes, actual scene playback/pause and every pose at two portrait sizes. The stronger `RemainingCast/Idles` candidate was rejected and is not an import source for these guards.

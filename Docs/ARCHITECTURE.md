...
- `BattleBackgroundTilemapController` owns world-background placement and clipping. Its generated rectangular SpriteMask follows the TopHUD interior inset by the native 16px frame and covers only the authored Default/-100 through -70 background orders. The board mask covers BoardBackground through WorldUI, excluding the background without changing board SpriteRenderer or UI sorting. Tilemaps retain authored unit scale, cell size and 64 PPU. The map origin is snapped to the 1/64 world grid after the Pixel Perfect Camera projection update, and the floor marker consumes that snapped baseline. The screen-space battle frame remains above the world background. Battle scenery uses only the Tilemap; the legacy section images, sprite floor settings, and fitted world sprite have been retired. `BattleBackgroundViewportValidation.Run` checks oversized-tile leakage, production board-mask isolation, scale and floor alignment at 1080x1920 and 1080x2400 in a graphics-enabled batch editor (omit `-quit`; the validation exits after its frame loop completes).

- `GameplayPixelLayoutController`, installed on the gameplay safe root by `RuntimeScenePresentationBootstrap` for every scene load, is the only writer of that root, `TopHUD`, `GameArea`, and `BottomHUD`. `SafeAreaFitter` is disabled on that root; other scenes may continue using it. Device constraints come from `UnityEngine.Device.Screen.width`, `height`, and `safeArea`: Unity's simulator-aware API, identical to `Screen` in device builds. All constraints enter the geometry calculation together. There are no guessed mobile bezel sizes or gap-limited HUD movements after fitting.
- The gameplay Canvas uses Constant Pixel Size at the largest positive integer scale fitting the native 448px minimum battle-content width plus safe padding. This minimum derives from the 146px player section, section/content margins, and three 80px enemy-slot allocations. Vertical feasibility (256px battle minimum, 176px bottom, two 6px gaps, and the board) can reduce that integer scale. The old 540x960 reference does not impose arbitrary scale thresholds. Canvas reference PPU remains 64. The safe viewport is rounded inward to this logical grid with four additional logical pixels on each edge. Decorative backgrounds may remain edge-to-edge; framed content uses the reported safe area plus pixel-grid padding.
- The stack has two gaps of at least six logical pixels, with a preferred gap of two native frame thicknesses (32px). The bottom enclosure starts at 176 logical pixels: two native 80px corners plus a 16px straight section. The 176×64 ability button sits 24 logical pixels below the enclosure center, flanked by 64×64 potion/Bomb slots at x=±136. The original energy art is rotated 90 degrees above the button with a runtime energy count. These controls fit inside the native borders without changing the enclosure or board geometry. The battle region has a 256px minimum, 320px preferred height and 480px maximum. When surplus height would create large empty gutters, the battle enclosure grows first, then the bottom enclosure can add up to one 64px art cell (240px maximum). Only tiled enclosure geometry grows; art and character scales remain governed by their existing pixel-grid consumers. TopHUD and BottomHUD anchor to the upper and lower safe viewport edges. The square board fills the available width/height and remains centered between them; remaining surplus height is split between the gaps with at most one logical pixel difference. Inward rounding plus four logical pixels of breathing room keeps the entire framed content inside the reported safe area.
...
- `PlayerHudCenteringController` is the final owner of the player sprite/gem/health-bar stack after responsive layout and pixel sizing. It keeps four logical pixels between their fixed containers, then centers their combined bounds inside PlayerPanel. A disabled legacy Image does not hide a container whose modular health-bar children still render. The stack uses full fixed animation bounds, never per-pose opaque bounds.
- Combat idle production sources live in `ArtSource/CombatIdles/`. Explicit editor helper `CombatIdleImporter.Run` copies the PNGs unchanged and authors Point/FullRect sprites and looping `Image.m_Sprite` clips under `Assets/_Game/Art/CombatIdles/` and `Assets/_Game/Animations/CombatIdles/`. All four loops contain nine 130 ms frames (1.17 seconds). All four use fixed 64×64 slices and bottom-center pivots; Bardley's source contact is now normalized to row 63 across idle and ability. Existing `PlayerBattleCharacterPresenter` and `EnemyVisualPresenter` consume the four definition references. Animation curves contain no transforms or combat events; gameplay/impact timing retains its existing owners. `CombatIdleValidation.Run` is an opt-in graphics-enabled batch Play Mode check of the production Game scene; see `Docs/Validation/COMBAT_IDLE_VALIDATION.md`.
- Combat actions live in `ArtSource/CombatActions/`. `CombatActionImporter.Run` extends the existing four controllers: 8-frame, 680 ms Farmer/Pan auto-attacks and 10-frame, 880 ms player ability casts, each returning to Idle. Enemy sheets are 96×64 with a 16-pixel symmetric canvas expansion; existing 64-pixel reference sizing preserves body scale and bottom-center floor. Player idle/cast slices are both 64×64.
- `PlayerAbilityController.AbilityActivated` emits once after runtime acceptance and successful energy spend; `PlayerBattleCharacterPresenter` consumes it for the optional Ability trigger. The presenter installs on subsequent scene loads as well as initial startup. State/energy notifications and persistent effects do not retrigger a cast. Gameplay effects still belong to the ability runtime.
- Farmer/Pan opt into `timeAutoAttackFromAnimation` and `useAuthoredAutoAttackMotion`. `EnemyCombatFeedback` skips its generic UI lunge for these definitions. `CharacterAnimationPlayback.AutoAttackImpact` relays the 320 ms event through `EnemyActionAnimationPresenter`, which resolves it in the same frame's LateUpdate after Unity has applied the Image sprite curve. The queued presentation ID rejects cancelled/replaced hits. Existing `EnemyAutoAttack` payload/ownership guards remain authoritative; a 670 ms completion event acknowledges recovery, and the clip ends at 680 ms. Scaled-clock timeouts pause with gameplay and still resolve/release missing-animation actions. A new accepted authored hit restarts its own clip, preventing a cancelled clip's late event from supplying a replacement attack's hit.
- `CombatActionValidation.ImportAndRun` is the opt-in graphics-enabled production-scene check for the above clips, events, accepted ability casts and every pose's portrait grounding/centering. It uses disposable account profiles, never the user's run.
- Local enemy sources live in `ArtSource/LocalEnemies/`. `CombatActionImporter.ImportLocalEnemies` uses the same importers to extend Miner, Basket Villager and Barricade Villager without reimporting the original four. Miner uses fixed 96×80 cells; the other two use 64×64 idles and 96×64 actions. All retain bottom-center pivots and the existing 64-pixel reference scale. `PixelPerfectBattleCharacterUI` compensates taller center-pivoted Image canvases on the actor root, preserving the layout-owned spawn anchor and feedback-owned VisualRoot. The offset is constant across the Miner's states and recomputed for resized pixel scales; it never follows the airborne silhouette. All three floor rectangles must agree and clear their health bars.
- These three enemies opt into authored auto-attack motion. Miner and Barricade Villager additionally opt into `useAuthoredSpecialAbilityMotion`: an accepted special receives an actor action ID and directly restarts Ability. The 360 ms contact is relayed after the sprite update; the 870 ms completion releases recovery. `EnemySpecialActionAvailability` waits for a settled board before starting these authored specials, including when two enemies become ready together. BoardController's existing mutation queue accepts optional impact gating for barricade placement as well as mining. It retains capacity checks, pause-aware three-second missing-impact fallback, cancelled-owner checks, and centralized environmental clear/refill. Authored barricades begin their existing materialization on contact; older untimed placement retains its prior visual sequence. Presenter completion fallback covers missing recovery events after contact.
- `LocalEnemyAnimationValidation.ImportAndRun` checks these eight clips in a graphics-enabled production Game scene with disposable profiles, including actual damage, mining/build contact, missing-event fallback, pause, cancellation and every pose at two portrait sizes.
- `CombatIdleImporter.ImportEnemyFamily` imports the selected nineteen enemy idles from Miner through King using `LocalEnemies/`, `GuardIdles/` and `RemainingCast/SelectedIdles/`. `ImportGuards` remains the four-guard subset. The existing nine-frame/130ms Point/FullRect import and definition-selected controllers remain authoritative; no runtime owner is added. Source names map SwordKnight to Enemy_Knight, RoyalMage to Enemy_CourtMage and RoyalArcanist to Enemy_RoyalArchbishop. Only visual references change; local action states/events and generic guard/knight/royal action feedback retain their existing timing. `CombatIdleValidation.ImportEnemyFamilyAndRun` verifies source bytes, actual scene playback/pause and all 171 poses at two portrait sizes. The stronger `RemainingCast/Idles` candidate remains rejected and is not an import source.
- Editor/development `GameplayPixelLayoutValidator` reports device/logical/physical rectangles, gaps, clearance, source-texel ratio, board origin, frame piece bounds, scale, missing pieces, and known competing layout components. It checks frame, artwork and text containment, bounded section heights, square board geometry, balanced gaps and Point-filtered imported pixel art. `GameplayPixelLayoutTests` provides 112 geometry and actual-scene cases (16 resolutions × seven safe areas), invalid-input and deliberate scene-violation checks, and a separate seven-profile Device Simulator run using the actual device API without an override. Fixtures and provenance live in `Scripts/Editor/PortraitDevices`; reports and screenshots go to `.utmp/PortraitLayout`. Screenshot inspection remains separate from numeric assertions.
...
- `BoardController.TileBursts` snapshots actual destroyed tile centers before clear and emits immutable `TileBurstVFXContext` batches at shatter. `TileBurstVFXController` pools centered native sprites; absent art retains existing procedural fallbacks. Overlapping families choose one deterministic material per cell; preserved rewards and protected crystals are excluded. The cue adds no gameplay wait. `GemsLanded` coalesces first gravity contacts per rendered movement frame, including completion after a long frame.
- `CombatAudioController` observes committed board/actor/ability events, groups cues, applies a six-voice priority/headroom budget, and suppresses pause/focus/recovery playback. Actor heal/shield/status notifications describe actual effects only. `MobileHaptics` coalesces short system-respecting Android/iOS feedback; unsupported devices omit it. These consumers never change combat or board state.
- `DungeonPresentationArt` decorates the existing layout: world masonry behind the masked scene, inset tiled HUD/player backgrounds, and menu art. `NativePixelArtwork` retains integer physical menu-art scaling. `DungeonPresentationArtImporter` dresses the existing environment prefab at its authored scale/sorting; it does not introduce a second scenery/layout owner.
...
## Balance v1 account and run boundary

`AccountProgression` owns a versioned `account-v1.json` under `Application.persistentDataPath`. Wallet, independent character levels, shared unlocks, inventory, equipment and the active run journal are separate fields. Mutations clone the next state, flush a temporary file, atomically replace/move the save, then publish state and notifications. A backup is retained. A failed write returns rejection before gameplay/item use proceeds. JsonUtility null inline-class behavior is normalized explicitly so an absent journal cannot become a phantom active run. Unsupported/corrupt files are preserved; a readable backup can recover the previous complete transaction.

Only explicit legacy mastery selections are grandfathered. Existing selection/preferences are not rewritten during migration. Disposable profile, selection and mastery scopes support tests without wiping the user's save. Temporary cards are stored only in the active run continuation, never as permanent account upgrades.

`AccountProgression.TryResetLevel` uses the same atomic save transaction to reset one known character to level 1 between runs. It rejects an active journal or an already-level-1 character, retains wallet/inventory/equipment/records and earned unlocks, and does not refund upgrade gold. Row/Column bombs and Color Crystals are always available. Poison/Healing/Shield first unlock at individual levels 3/5/7, and the additive account unlock list retains earned access after resets or threshold changes. Character-select feedback compares actual ownership before and after an upgrade so releveling cannot announce an old unlock as new.

`RunSession` begins one journal before player initialization, snapshots permanent level/charges/mastery, records sequential completed waves, and gates progression if a journal write fails. `OriginalEncounterDefinitions` captures initial successfully spawned members, including actors defeated during entrance; summons never add income. Rewards are previewed separately and credited once on death, victory or explicit End Run/Retry. Suspend, application quit and current-version interruption preserve the attempt. Only legacy journals lacking continuation state settle on load. Partial waves pay nothing. The UI exposes save retry without inventing a second reward calculation.

Potion healing uses `PlayerActor.Heal` plus the established healing resolver. Bomb acceptance locks the board before committing inventory, then uses existing bomb expansion, clear reporting, shatter activation, crystal chains, collapse/cascade and emergency reshuffle. Invalid targets/cancellation never spend. Consumable clears do not count as accepted matching moves. Board ownership continues through settlement; VFX and settings never own resolution.

Permanent HP/gem/ability/shield values derive from immutable `PlayerDefinition` data at the journal's run-start level, then `RunUpgradeResolver` applies temporary modifiers. Bardley's serialized cost is now 80. `AbilityRefundBudget`, owned by `PlayerAbilityMatchEnergyGain`, caps generated energy from the complete accepted cast at half the effective cost. The runtime and board must both finish before the budget ends. `PlayerAbilityController` still spends only after acceptance. Wave-start energy remains separate from cast refunds.
...
`EncounterRecipe` adds editable whole formations to the existing spawn profile. Ordinary random rolls and fallback fills respect whole-formation threat, support/disruptor limits, eligibility and narrative escorts. Named leaders are excluded after appearance; the required Archbishop escort is the explicit exception. Mining/barricade candidates reserve cells and test legal moves before commitment; shared structural limits are rechecked when queued mutations execute. Crossbow and Captain use the same movable-pin pipeline; Mage freezes retain their distinct state and breaking semantics.

`RunControlsUI` owns only its pause and external-input token. It restores the prior time scale only if its own pause is still active and disposes only its own token. Confirmed exits settle account state before scene loading. `AudioPreferences` stores independent music, SFX and vibration preferences; each consumer applies its own state. `Resources/UI/Typography.asset` references both the existing Liberation Sans TTF and TMP SDF for a replaceable non-pixel family. Current polished consumable sources are under `ArtSource/Presentation` (earlier sources remain in `ArtSource/Consumables`); quantity/cooldown labels are runtime text.

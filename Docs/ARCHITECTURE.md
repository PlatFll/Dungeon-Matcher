...
- The stack has two gaps of at least six logical pixels, with a preferred gap of two native frame thicknesses (32px). The bottom enclosure starts at 176 logical pixels: two native 80px corners plus a 16px straight section. A centered 64px ability button has at least 40px vertical breathing room inside the 16px borders. The battle region has a 256px minimum, 320px preferred height and 480px maximum. When surplus height would create large empty gutters, the battle enclosure grows first, then the bottom enclosure can add up to one 64px art cell (240px maximum). Only tiled enclosure geometry grows; art and character scales remain governed by their existing pixel-grid consumers. TopHUD and BottomHUD anchor to the upper and lower safe viewport edges. The square board fills the available width/height and remains centered between them; remaining surplus height is split between the gaps with at most one logical pixel difference. Inward rounding plus four logical pixels of breathing room keeps the entire framed content inside the reported safe area.

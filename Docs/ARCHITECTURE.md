...
`RunControlsUI` owns only its pause and external-input token. It restores the prior time scale only if its own pause is still active and disposes only its own token. Confirmed exits settle account state before scene loading. `AudioPreferences` stores two independent PlayerPrefs settings; music and gem-break SFX consumers apply their corresponding mute state. `Resources/UI/Typography.asset` references both the existing Liberation Sans TTF and TMP SDF for a replaceable non-pixel family. Editable consumable art is under `ArtSource/Consumables`; quantity/cooldown labels are runtime text.

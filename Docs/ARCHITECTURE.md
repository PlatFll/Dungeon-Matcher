...
Only explicit legacy mastery selections are grandfathered. Existing selection/preferences are not rewritten during migration. Disposable profile, selection and mastery scopes support tests without wiping the user's save. Temporary cards are never persisted into account data.

`RunSession` begins one journal before player initialization, snapshots permanent level/charges, records sequential completed waves, and gates progression if a journal write fails. `OriginalEncounterDefinitions` captures initial successfully spawned members, including actors defeated during entrance; summons never add income. Rewards are previewed separately and credited once on death, victory, Retry, menu exit, application quit or next-load interruption recovery. Partial waves pay nothing. The UI exposes save retry without inventing a second reward calculation.
...
Permanent HP/gem/ability/shield values derive from immutable `PlayerDefinition` data at the run-start level, then `RunUpgradeResolver` applies temporary modifiers. The development Bardley asset retains energy cost 1. Production-cost playthroughs use disposable clones; no shared asset is mutated by a test run.

...
- The stack has two six-logical-pixel gaps. The bottom enclosure is 176 logical pixels: two native 80px corners plus a 16px straight section. A centered 64px ability button has 40px vertical breathing room inside the 16px borders. The battle region has a 220px minimum, 290px preferred height, and 320px upper bound; the current policy stops at the preferred height. Surplus height is background around the centered stack.
- `BoardLayoutController` consumes the assigned `GameArea`. Its render callback is registered in Start, after URP Pixel Perfect Camera's OnEnable callback, so it measures the actual updated projection before fitting. It floors the available physical source-texel ratio and converts that ratio to world scale using the current camera mapping and Assets PPU (64). The board center is snapped in physical screen coordinates. PPC's per-renderer world-grid snapping stays off: that separate 1/64-world-unit grid conflicts with a board scaled by 0.5 under a 2x camera projection. Render scale remains 1 and MSAA remains off.
- **Approved narrow-display exception:** the existing 8x8 board and 16px borders require 544 physical pixels at 1x, or 552px including the minimum side insets. Below 552px of safe width, the board alone may downscale fractionally to fit. The validator reports this exception explicitly. UI still uses an integer scale. Wider displays may not silently take that fallback; an infeasible viewport is reported as an error.

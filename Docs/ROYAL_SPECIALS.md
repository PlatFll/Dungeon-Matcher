...
- The standard participates in board gravity. When gem space opens below it, it falls through valid gravity destinations with the authoritative board resolution rather than using a turn timer.
...
- Royal standards are non-gem occupants tracked by `BoardController.RoyalBanners`. Their movement is coordinated with actual board openings, and presentation never owns their gameplay lifetime.

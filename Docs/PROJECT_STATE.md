...
`ArtSource/CombatIdles/` contains edited Rattlebones, Farmer, Pan Villager and Bardley idle sources, each with nine 64×64 frames at 130 ms (1.17 seconds). The refinement in [PR #159](https://github.com/PlatFll/Dungeon-Matcher/pull/159) gives Farmer a rigid pitchfork carried by both grips and restores the original's distinct head/hat/face nod poses over that improved body motion. Bardley's existing face pixels follow its body in steps of at most one pixel. The latest Farmer-only head correction preserves the preceding torso, hands, shaft and feet; Rattlebones, Pan Villager and Bardley are unchanged by that correction. Every frame uses its approved material palette, with the shared `#0A0D11` outline and binary transparency. See the [delivery notes](../ArtSource/CombatIdles/README.md), [Farmer comparison](../ArtSource/CombatIdles/FarmerHeadReview.html), [frame/export checks](../ArtSource/CombatIdles/Validation.json), and [head preservation checks](../ArtSource/CombatIdles/FarmerHeadValidation.json).

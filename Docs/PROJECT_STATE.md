...
`ArtSource/CombatIdles/` contains edited Rattlebones, Farmer, Pan Villager and Bardley idle sources, each with nine 64×64 frames at 130 ms (1.17 seconds). The refinement in [PR #159](https://github.com/PlatFll/Dungeon-Matcher/pull/159) gives Farmer a calmer two-pixel head motion and a rigid pitchfork carried by both grips, and keeps Bardley's existing face pixels moving with the body in steps of at most one pixel. Rattlebones's reference motion and Pan Villager's upright stance dip are unchanged by the refinement. Every frame uses its approved material palette, with the shared `#0A0D11` outline and binary transparency. See the [delivery notes](../ArtSource/CombatIdles/README.md), [frame/export checks](../ArtSource/CombatIdles/Validation.json), and [motion checks](../ArtSource/CombatIdles/RefinementValidation.json).
